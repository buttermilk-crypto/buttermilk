...
class Sha1 extends Digest {
	...
	/**
	 * Get the length of the hash output, in bytes. For SHA-1, this will always
	 * return 20.
	 */
	...
	public void update(byte[] data, int offset, int length) {
		...
		if (offset + length > data.length)
			throw new IllegalArgumentException("reading " + length
					+ " bytes of input starting at offset " + offset
					+ " will overrun end of input buffer (" + data.length
					+ " bytes long)");
		if (offset < 0)
		...
		if (outOffset + getDigestLen() > out.length)
			throw new IllegalArgumentException(
					"output array is too short: start at offset " + outOffset
							+ " and writing " + getDigestLen()
							+ " bytes will overrun " + " buffer length "
							+ out.length);
		if (outOffset < 0)
	...
	/**
	 * A temporary buffer used by the transform() routine It is allocated once
	 * per object to reduce the cost of allocation.
	 */
	private int[] data = new int[16];
	private int[] w = new int[16];

	/**
	 * Convert an array of bytes into an array of 32-bit integers. Each output
	 * integer uses 4 input bytes. The number of input bytes must be a multiple
	 * of 4.
	 */
	private static void byte2int(byte[] src, int srcOffset, int[] dst,
			int dstOffset, int numInts) {
		while (numInts-- > 0) {
			// Big endian
	...
	/**
	 * Convert an integer array into an byte array using the big-endian encoing
	 * of the integers.
	 */
	protected void int2byte(int[] src, int srcOffset, byte[] dst,
			int dstOffset, int numInts) {
