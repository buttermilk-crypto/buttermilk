...
class PrivKeyFormatter_PrivateKeyListedFv1 implements PrivKeyFormatter {
	...
	public byte[] encode(KeyParams keyParams, FullPolynomial h, FullPolynomial f) {
		...
		int len = (KeyFormatterUtil.fillHeader(tag, keyParams.OIDBytes, null)
				+ BitPack.pack(keyParams.N, keyParams.q) + BitPack.pack(
				2 * keyParams.df, keyParams.N));
		...
		offset += KeyFormatterUtil.packListedCoefficients(F, keyParams.df,
				keyParams.df, ret, offset);
		return ret;
	}
	...
	public RawKeyData decode(byte keyBlob[])
		...
		offset += BitPack.unpack(keyParams.N, keyParams.q, keyBlob, offset,
				h.p, 0);

		// Recover F
		FullPolynomial f = new FullPolynomial(keyParams.N);
		offset += KeyFormatterUtil.unpackListedCoefficients(f, keyParams.N,
				keyParams.df, keyParams.df, keyBlob, offset);
		// Compute f = 1+p*F
