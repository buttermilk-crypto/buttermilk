...
class PubKeyFormatter_PUBLIC_KEY_v1 implements PubKeyFormatter {
	...
	public byte[] encode(KeyParams keyParams, FullPolynomial h) {
		...
		int len = (KeyFormatterUtil.fillHeader(tag, keyParams.OIDBytes, null) + BitPack
				.pack(keyParams.N, keyParams.q));
	...
	public RawKeyData decode(byte keyBlob[])
		...
		if (headerLen + packedHLen != keyBlob.length)
			throw new IllegalArgumentException("Input public key blob is "
					+ keyBlob.length + " bytes, not " + "the expected "
