...
public class KeyFormatterUtil {
	...
	static KeyParams parseOID(byte keyBlob[], int oidStartIndex, int oidLen)
			throws ParamSetNotSupportedException {
		if (oidStartIndex + oidLen > keyBlob.length)
			throw new IllegalArgumentException(
					"keyblob not large enough to hold OID");
	...
	static public byte[] packListedCoefficients(FullPolynomial F, int numOnes,
			int numNegOnes) {
	...
	static public int packListedCoefficients(FullPolynomial F, int numOnes,
			int numNegOnes, byte out[], int offset) {
		if (out == null)
		...
		int len = BitPack.pack(numOnes + numNegOnes, F.p.length, coefficients,
				0, out, offset);
		java.util.Arrays.fill(coefficients, (short) 0);
		return len;
	}

	static public int unpackListedCoefficients(FullPolynomial F, int N,
			int numOnes, int numNegOnes, byte in[], int offset) {
		short coefficients[] = new short[numOnes + numNegOnes];
		int len = BitPack.unpack(coefficients.length, N, in, offset,
				coefficients, 0);
