 ...
 * Cryptosystems Technical Report #14 "Almost Inverses and Fast NTRU Key
 * Creation".
 *
 * <p>
 * The prime modulus is supplied to the constructor, as a result each instance
 * of this class can compute inverses for one modulus.
 *
 * <p>
...
public class PolynomialInverterModPrime implements PolynomialInverter {
	...
	/**
	 * A table of inverses mod <code>prime</code>. The table should be set up so
	 * that invModPrime[i] * i = 1 (mod prime) if the inverse of i exists
	...
	/**
	 * Compute the inverse of a polynomial in (Z/pZ)[X]/(X^N-1) See NTRU
	...
	/**
	 * Returns x mod prime, always in the range [0..prime-1]. This differs from
	 * the % operator by returning a positive result when fed a negative number.
	 */
	...
	/**
	 * Divide f(X) by the polynomial g(X)=X. f(X) is updated in-place. This
	 * effectively a "shift-left-with-wraparound" of the indices.
	 */
	...
	/**
	 * Mulitply f(X) by the polynomial g(X)=X. f(X) is updated in-place. This
	 * effectively a "shift-right-with-wraparound" of the indices.
	 */
