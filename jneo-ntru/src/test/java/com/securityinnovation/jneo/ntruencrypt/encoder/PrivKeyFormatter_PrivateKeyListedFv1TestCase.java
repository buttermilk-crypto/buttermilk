...
public class PrivKeyFormatter_PrivateKeyListedFv1TestCase {
	...
	byte[] buildPrivBlob(KeyParams keyParams, byte packedH[],
			byte packedListedF[]) {
	...
	FullPolynomial recoverf(short fBytes[], int q) {
	...
	@Test
	public void test_encodePrivKey() throws NtruException {
		for (int t = 0; t < tests.length; t++) {
			...
			byte expected[] = buildPrivBlob(keyParams, tests[t].packedH,
					tests[t].packedListedF);

			System.err.println(Arrays.toString(privBlob));
	...
	public void test_decodePubKey() throws NtruException {
		for (int t = 0; t < tests.length; t++) {
			...
			// Build a blob
			byte blob[] = buildPrivBlob(keyParams, tests[t].packedH,
					tests[t].packedListedF);
	...
	public void test_decodePubKey_bad_oid() throws NtruException {
		...
		try {
			...
			// Build a blob
			blob = buildPrivBlob(keyParams, tests[t].packedH,
					tests[t].packedListedF);
			// Change the oid
			blob[2] = (byte) 0xff;
	...
	public void test_decodePubKey_shortBuf() throws NtruException {
		...
		try {
			...
			byte blob[] = buildPrivBlob(keyParams, tests[0].packedH,
					tests[0].packedListedF);
	...
	public void test_decodePubKey_longBuf() throws NtruException {
		...
		try {
			...
			fail("setup for decoding long blob nexpectedly threw "
					+ t.toString());
		}
