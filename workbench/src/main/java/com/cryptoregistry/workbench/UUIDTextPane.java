...
public class UUIDTextPane extends JTextPane implements ActionListener {
	...
	private static final Pattern DoubleQuotesPat = Pattern
			.compile("\"([^\"]*)\"");
	private static final String[] topLevelTokens = { "Contacts", "Data",
			"Local", "Keys", "Macs", "Signatures" };
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "validate": {
			if (isValidJSON(getText())) {
				JOptionPane.showMessageDialog(comp, "Valid JSON",
		...
		case "edit-attributes": {
			...
			String text = this.getText().substring(0, pos + 20); // HACK - but
																	// should
																	// contain
																	// what we
																	// want
			...
			while (matcher.find()) {
				list.add(new Match(matcher.start(1), matcher.end(1), matcher
						.group(1)));
			}
			check(1, list);
		}
	...
	public void insert(CryptoSignature sig) {
		...
		try {
			this.getDocument().insertString(this.getCaretPosition(), result,
					null);
		} catch (BadLocationException e) {
			e.printStackTrace();
		}
	...
	class UndoManager extends AbstractUndoableEdit implements
