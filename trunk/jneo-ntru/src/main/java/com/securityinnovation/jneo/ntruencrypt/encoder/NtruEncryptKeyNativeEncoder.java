...
public class NtruEncryptKeyNativeEncoder implements NtruEncryptKeyEncoder {
	 ...
	 * blob type tag == PUBLIC_KEY_v1 1 byte OID length <> bytes OID <> bytes h,
	 * bit packed.
	 */
	 ...
	 * bytes OID <> bytes h, bit packed. <> bytes F, packed 5 trits per output
	 * byte: out[i] = F[5*i]*3^4 + F[5*i+1]*3^3 + F[5*i+2]*3^2 + F[5*i+3]*3 +
	 * F[5*i+4]
	 */
	...
	PrivKeyFormatter pickDefaultPrivKeyFormatter(KeyParams keyParams) {
		...
		int packedListedFLength = (keyParams.df
				* 2
				* com.securityinnovation.jneo.math.BitPack
						.countBits(keyParams.q + 7) / 8);
	 ...
	 */
	public byte[] encodePrivKey(KeyParams keyParams, FullPolynomial h,
			FullPolynomial f) {
	 ...
	 */
	public RawKeyData decodeKeyBlob(byte keyBlob[])
		...
		case (PRIVATE_KEY_DEFAULT_v1): {
			PrivKeyFormatter formatter = pickDefaultPrivKeyFormatter(KeyFormatterUtil
					.parseOID(keyBlob, 1, 3));
			return formatter.decode(keyBlob);
		}
		}
