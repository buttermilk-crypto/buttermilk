...
/**
 * This class implements an InputStream whose output conforms to the X9.82
 * specification for "Hash Function DRBG Using Any Approved Hash Function"
 * (section 10.1.3 of ANS X9.82, Part 3, Draft July 2003).
 *
 * <p>
 * In addition to the InputStream API, this class also adds functions for
 * seeding and reseeding the DRBG.
 */
public class X982Drbg extends InputStream {
	...
	private final static byte t[] = { (byte) 0xcf, (byte) 0x83, (byte) 0xe1,
			(byte) 0x35, (byte) 0x7e, (byte) 0xef, (byte) 0xb8, (byte) 0xbd,
			(byte) 0xf1, (byte) 0x54, (byte) 0x28, (byte) 0x50, (byte) 0xd6,
			(byte) 0x6d, (byte) 0x80, (byte) 0x07, (byte) 0xd6, (byte) 0x20,
			(byte) 0xe4, (byte) 0x05, (byte) 0x0b, (byte) 0x57, (byte) 0x15,
			(byte) 0xdc, (byte) 0x83, (byte) 0xf4, (byte) 0xa9, (byte) 0x21,
			(byte) 0xd3, (byte) 0x6c, (byte) 0xe9, (byte) 0xce, (byte) 0x47,
			(byte) 0xd0, (byte) 0xd1, (byte) 0x3c, (byte) 0x5d, (byte) 0x85,
			(byte) 0xf2, (byte) 0xb0, (byte) 0xff, (byte) 0x83, (byte) 0x18,
			(byte) 0xd2, (byte) 0x87, (byte) 0x7e, (byte) 0xec, (byte) 0x2f,
			(byte) 0x63, (byte) 0xb9, (byte) 0x31, (byte) 0xbd, (byte) 0x47,
			(byte) 0x41, (byte) 0x7a, (byte) 0x81, (byte) 0xa5, (byte) 0x38,
			(byte) 0x32, (byte) 0x7a, (byte) 0xf9, (byte) 0x27, (byte) 0xda,
			(byte) 0x3e };

	/**
	 * Implement the Initialize_Hash_DRBG algorithm from the X9.82 spec. This
	 * implementation does not support an application purpose (the variable t).
	 * It also does not enforce the minimum entropy requirements, or checks on
	...
	/**
	 * Implement the Reseed_Hash_DRBG algorithm from the X9.82 spec. This
	 * implementation does not support an application purpose (the variable t).
	 * It also does not enforce the minimum entropy requirements.
	 */
	...
	/**
	 * Implement the Hash_DRBG algorithm from the X9.82 spec. This
	 * implementation does not support any user input.
	 */
	public int read() {
		byte b[] = new byte[1];
		read(b, 0, 1);
	...
	public int read(byte[] out, int offset, int len) {
		...
		if (offset + len > out.length)
			throw new IllegalArgumentException("Writing " + len
					+ " bytes of output starting at offset " + offset
	...
	/**
	 * Implement the Hashgen subroutine of the X9.82 Hash_DRBG algorithm:
	 * calculate H(V) | H(V+1) | H(V+2) | ... until len bytes are generated.
	 * Store the result in out[offset...].
	 */
	...
	/**
	 * Add x to a multi-byte big-endian integer: accum += x. Overflow is
	 * discarded.
	 */
	...
	/**
	 * The seed vector, called V in the X9.82 spec. Updated in each call to
	 * generate random bytes.
	 */
	private byte[] V;

	/**
	 * This is a copy of V that is incremented during the internal hashGen
	 * method. It is preallocated once per DRBG to avoid the overhead of
	 * allocation.
	 */
	private byte[] Vtmp;

	/**
	 * A constant derived from the seed. Constant for the life of the seed.
	 */
	private byte[] C;
