...
/**
 * This class represents NtruEncrypt keys. An instance may hold a public/private
 * keypair or just the public key.
 */
public class NtruEncryptKey {
	...
	/**
	 * The private key f (= 1+p*F). For public-key-only objects this will be
	 * null.
	 */
	protected FullPolynomial f = null;

	/**
	 * Generate a new keypair for the specified parameter set. (see
	 * com.securityinnovation.jNeo.OID for a list of parameter set identifiers).
	 */
	public static NtruEncryptKey genKey(OID oid, Random prng)
		...
		return genKey(oid, prng.rng);
	}

	/**
	 * Generate a new keypair for the specified parameter set using the supplied
	 * InputStream as a source of randomness.
	 */
	static NtruEncryptKey genKey(OID oid, InputStream prng)
			throws ParamSetNotSupportedException {
	 ...
	 */
	public NtruEncryptKey(KeyParams _keyParams, FullPolynomial _h, FullPolynomial _f) {
	...
	/**
	 * Create a new key from the supplied keyblob. The key blob can be either a
	 * public or private key blob generated with <code>getPubKey()</code> or
	...
	/**
	 * Create an empty key object associated with the supplied parameter set.
	 * This is for use in unit testing where the key material is not used.
	 */
	NtruEncryptKey(OID oid) throws ParamSetNotSupportedException {
		keyParams = KeyParams.getKeyParams(oid);
	}

	/**
	 * Signal that this app is finished using this key. This clears all
	 * sensitive memory stored by the object and makes the object unusable.
	 */
	public void close() {
		if (h != null)
	 ...
	 */
	public byte[] getPubKey() throws ObjectClosedException {
		if (h == null)
	 ...
	 */
	public byte[] getPrivKey() throws ObjectClosedException,
	 ...
	 */
	protected byte[] encrypt(byte message[], InputStream rng)
			throws ObjectClosedException, PlaintextBadLengthException {
		if (h == null)
			throw new ObjectClosedException();
		// Sanity check input
		if (message == null)
			throw new NullPointerException(
					"NtruEncrypt input plaintext is NULL");
		...
		do {
			...
			// calc m' = M + mask mod p
			mPrime = FullPolynomial
					.addAndRecenter(Mtrin, mask, keyParams.p, -1);
	...
	public byte[] decrypt(byte ciphertext[]) throws NoPrivateKeyException,
		...
		if (ciphertext == null)
			throw new NullPointerException(
					"NtruDecrypt input ciphertext is NULL");
		if (h == null)
			throw new ObjectClosedException();
		if (f == null)
			throw new NoPrivateKeyException();
	 ...
	 */
	byte[] form_sData(byte m[], int mOffset, int mLen, byte b[], int bOffset) {
	...
	public FullPolynomial getH() {
		return h;
	}

	public FullPolynomial getF() {
		return f;
	}
