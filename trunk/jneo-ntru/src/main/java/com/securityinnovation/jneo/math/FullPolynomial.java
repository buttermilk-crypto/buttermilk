...
/**
 * This class provides an abstraction of a polynomial. Instances contain an
 * array of shorts that hold the polynomial coefficients and the class provides
 * operations to add, subtract, and multiply polynomials together.
 */
public class FullPolynomial {
	...
	/**
	 * Initialize a polynomial with the given coefficient list. The degree of
	 * the polynomial will be defined by the size of coeffs.
	 *
	 * @param coeffs
	...
	/**
	 * Recenter the coefficients of a polynomial into the range
	 * [newLowerLimit..newLowerLimit+q), such that the new coefficients equal
	 * the old coefficients mod q.
	 *
	 * @param a
	 ...
	 */
	public static final void recenterModQ(FullPolynomial a, int q,
			int newLowerLimit) {
	...
	/**
	 * Given two polynomials of the same degree N, compute a*b in the ring of
	...
	/**
	 * Given two polynomials of the same degree N, compute a*b in the ring of
	 * polynomials of degree N whose coefficients are in the ring of integers
	 ...
	 *            the modulus for the coefficients of the resulting polynomial.
	 *
	 * @return a*b modulo X^N.
	 */
	public static FullPolynomial convolution(FullPolynomial a,
			FullPolynomial b, int coefficientModulus) {
	...
	/**
	 * Add two polynomials modulo coefficientModulus. The coefficients of the
	 ...
	 */
	public static final FullPolynomial add(FullPolynomial a, FullPolynomial b,
	...
	/**
	 * Add two polynomials modulo coefficientModulus. The coefficients of the
	 * resulting polynomial will be in the range
	...
	public static final FullPolynomial addAndRecenter(FullPolynomial a,
			FullPolynomial b, int coefficientModulus, int newLowerLimit) {
		FullPolynomial c = new FullPolynomial(a.p.length);
		for (int i = 0; i < c.p.length; i++)
			c.p[i] = (short) (a.p[i] + b.p[i]);
	...
	/**
	 * Subtract two polynomials modulo coefficientModulus. The coefficients of
	 * the resulting polynomial will be in the range [0..coefficientModulus-1].
	 *
	 * @param a
	 *            the minuend.
	 * @param b
	 ...
	 */
	public static final FullPolynomial subtract(FullPolynomial a,
			FullPolynomial b, int coefficientModulus) {
		FullPolynomial c = new FullPolynomial(a.p.length);
		for (int i = 0; i < c.p.length; i++)
			c.p[i] = (short) (a.p[i] - b.p[i]);
	 ...
	 *
	 * @return true iff o is a FullPolynomial with the same degree as this
	 *         object and the respective coefficient lists contain the same
	 *         values.
	 */
